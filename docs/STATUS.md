...
- O gerador de cortesias carrega as viagens mesmo se `limite_cortesia` estiver pendente; nesse caso somente a emissão fica bloqueada.
- Cadastros > Encomendas ganhou labels explícitas na tabela por trecho. O valor fixo vale até `limiteValorFixo`; o percentual é aplicado somente acima dele.
- O teto de encomenda passou a ser `limitePesoEncomenda`, editável na configuração. Acima dele o backend orienta lançar como carga. A migration preserva 30 kg como valor inicial editável, não como constante de código.
- A migration 0039 amplia `item_preco.tamanho` de `char(1)` para `varchar(24)`, eliminando o erro interno ao publicar códigos configuráveis maiores que um caractere.
- QA: migrations locais 39/39, build NestJS no WSL e build TanStack/Vite/Nitro verdes. A suíte Jest completa excedeu 120 s sem resultado e não foi declarada verde.
- Deploy: aplicar 0038 e 0039 antes da API/front. Não executar o seed mínimo em banco com vendas existentes, pois itens de preço já referenciados não podem ser substituídos.

## Trabalho 2026-08-05 - Integração real de BP-e via NS Tecnologia

- Provedor decidido e implementado: NS BP-e API. O AJC não fala SOAP diretamente com a SEFAZ e não guarda PFX/senha; o certificado corporativo fica no cofre/painel da NS e os secrets ficam no Coolify.
- Migration `0037_bpe_integracao_ns.sql` aplicada no PostgreSQL local. Ela adiciona código IBGE às cidades, estados/metadados reais do BP-e, numeração segura por CNPJ/ambiente/série, webhook idempotente, permissões fiscais e a configuração `vendas_bpe_integracao` desativada por padrão.
- Vendas individuais, PDV e portal deixaram de gerar `stub_emitido`: criam pendência fiscal real na mesma transação. Também foi corrigido o uso da coluna inexistente `servico`; o contrato correto é `provider`.
- Worker pg-boss ganhou a fila `fiscal-bpe-emitir`, varredura da outbox, trava por documento e lease transacional contra emissão concorrente, emissão NS, consulta de processamento, retry, rejeição, contingência e armazenamento de XML/DABPE no bucket privado `bpe-documentos` com SHA-256.
- API fiscal criada para readiness, consulta por bilhete, reprocessamento, download assinado, cancelamento e webhook NS com Basic Auth/idempotência. A preparação do cancelamento é transacional, guarda o XML do evento e exige permissão/justificativa; credencial inválida do webhook responde 401.
- Cadastros › Cidades edita e pesquisa código IBGE. Cadastros › Configurações operacionais ganhou painel BP-e com ambiente, série, número inicial, emitente, percursos, classes, pagamentos, componente, tributação e retry; token/PFX não aparecem no navegador.
- Segurança/deploy: compose e `.env.coolify.example` documentam `BPE_NS_*` e webhook. ADR 04, runbook `docs/deploy/NS-BPe-Onboarding.md`, módulo de Vendas, certificado, buckets e migrations foram atualizados.
- QA: migration 37/37; build Nest verde; 14 suites/47 testes Jest verdes; build TanStack/Vite/Nitro/Vercel verde. Inspeção autenticada em desktop e 390×844 validou BP-e e Cidades sem overflow horizontal; os únicos logs observados eram do redirecionamento inicial sem token, anteriores ao login. Nenhuma chamada real à NS foi feita sem token/conta de homologação.
- Próximo passo externo: contratar/liberar NS BP-e API, subir o PFX no painel seguro, obter token, confirmar série exclusiva/próximo número e códigos fiscais com contador/NS, preencher Cadastros e executar a homologação antes de produção.
...
## Entrega 2026-08-06 - Etapa 11: Financeiro operacional completo

- A proposta aprovada foi executada. A migration `0040_financeiro_operacional_completo.sql` amplia o nucleo 0015 sem remover registros legados: competencia, liquidacao parcial, historico de transicoes, plano de contas, centros de custo, comissoes condicionadas ao recebimento e controle interno de faturas.
- `/api/caixa` agora oferece filtros paginados de AP/AR, resumo por periodo, liquidacao idempotente, historico, comissoes (`em_aberto -> liberada -> pago`), DRE em caixa/competencia, faturas e CRUD de plano/centro. O backend aplica as permissoes granulares e registra auditoria nas transicoes sensiveis.
- `/app/financeiro` foi conectado aos contratos reais: caixas, KPIs, periodo, busca, AP, AR, comissoes, DRE, faturas, estados vazios/erro e formularios de lancamento. Cadastros ganhou `OperationalFinanceConfig` para publicar versoes de regras, categorias, modo da DRE, rastreamento de faturas, plano de contas e centros de custo; nenhum preco, percentual ou conta nasce hard-coded.
- Integracoes externas de rastreamento de faturas continuam bloqueadas ate fornecedor/credencial real; o controle interno ja esta disponivel. Conciliacao bancaria e Compras permanecem fora desta etapa, conforme o escopo aprovado (Compras = Etapa 12).
...
- A imagem de build da Vercel nao disponibiliza `npm` (exit 127). O comando voltou para Bun e o build passou a remover explicitamente `node_modules/.nitro` e `.vercel/output` antes do Vite/Nitro, pois o log provou que o cache interno preservava o mesmo chunk `server-Bo4wBqd-` de 171,24 kB mesmo com dependencias reinstaladas e build cache externo desativado.
- A Root Directory inclui dependencias externas e a Vercel usa efetivamente o `bun.lock` da raiz do monorepo. Foram fixados e alinhados `@lovable.dev/vite-tanstack-config 2.10.0`, `@tanstack/react-start 1.168.34`, `@tanstack/react-router 1.170.21`, `@tanstack/router-plugin 1.168.26` e `vite 8.2.1` nos locks raiz/standalone; a arvore antiga usava Lovable 2.5.3 e gerava deterministicamente o chunk quebrado.
- Mesmo com a toolchain alinhada, o runtime Node da Vercel recebeu o alias compartilhado `r as __exportAll` como valor nao-funcao. `scripts/fix-vercel-rolldown-runtime.mjs` agora isola esse helper nos chunks afetados apos o build; o script falha explicitamente se o formato do bundle mudar e nenhum alvo for encontrado.
- O painel Vercel tambem foi alinhado para executar `build:vercel`; redeploys de deployments antigos preservam seus overrides historicos, portanto a aplicacao efetiva exige deployment originado de commit novo.
## Trabalho 2026-08-07 - Hotfix final do helper SSR no bundle Vercel
- **Incidente:** deployments TanStack Start retornavam HTTP 500 com `TypeError: __exportAll is not a function` em chunks SSR da Vercel.
- **Diagnóstico confirmado:** além do import direto do `_runtime.mjs`, o bundle podia importar o mesmo helper com alias (`__exportAll$1`) a partir de outro chunk. O pós-build anterior não cobria esse caminho.

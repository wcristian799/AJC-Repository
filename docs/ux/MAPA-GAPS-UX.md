...
3. **Veículos** — módulo inteiro (4 telas) sem nada.
...
- **SPEC 02 parte C:** fluxo ✅ · conta cliente ✅ · telas ✅. **C.7 emissão fiscal 🔶** (pendência Lucas/contador, arquitetura prevista).
...
| ⚪ Fora MVP | Veículos | módulo inteiro | Fase posterior |
...
2. **Criar módulo Encomendas** (B.2 declaração de conteúdo é risco jurídico; B.4 controle por viagem).
3. **Completar telas operacionais de Vendas** (B.6 cortesias, B.8 manifesto de passageiros).
4. **Refinos** nas telas de gestão já boas (botões funcionais, painel do agente).
5. **Pendências externas** seguem destravando o resto: tabela de preços de encomenda (Lucas) e gateway+BP-e (portal).

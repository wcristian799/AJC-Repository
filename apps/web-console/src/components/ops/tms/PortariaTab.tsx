...
import { type RegistroPortariaTipo } from "@/mocks/data";

const TIPO_LABEL: Record<RegistroPortariaTipo, string> = {
  veiculo_carga: "Veículo de carga",
  veiculo_transporte: "Veículo p/ transporte",
  pessoa: "Pessoa",
};
...
function SimuladorPortaria() {
  ...
  const [tipo, setTipo] = useState<RegistroPortariaTipo>("veiculo_carga");
          ...
          <motion.div key="home" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="flex h-[calc(100%-44px)] flex-col p-5">
            ...
            <div className="mt-6 grid gap-3">
              ...
              <button className="surface-card flex h-24 flex-col items-center justify-center gap-2 text-foreground active:scale-[0.98]">
                ...
                <span className="text-[10px] text-muted-foreground">seleciona da lista “no pátio”</span>
              </button>
            </div>

            <p className="mt-auto text-[10px] text-muted-foreground">Data/hora são carimbadas automaticamente pelo dispositivo.</p>
          </motion.div>
        )}
        ...
        {screen === "entrada" && (
          <motion.div key="entrada" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex h-[calc(100%-44px)] flex-col p-5">
            ...
            <div className="mt-3 grid grid-cols-3 gap-1.5">
              {(["veiculo_carga", "veiculo_transporte", "pessoa"] as RegistroPortariaTipo[]).map((t) => (
                <button
                  key={t}
                  onClick={() => setTipo(t)}
                  className={`rounded-lg px-2 py-2.5 text-[10px] font-medium leading-tight ring-1 transition-colors ${
                    tipo === t
            ...
            <div className="mt-3">
              <label className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground">{tipo === "pessoa" ? "Nome" : "Empresa"}</label>
              <input
                value={empresa}
                onChange={(e) => setEmpresa(e.target.value)}
                placeholder={tipo === "pessoa" ? "Nome do visitante" : "Autocomplete · digite ou cadastre"}
            ...
            <button
              ...
              disabled={(tipo !== "pessoa" && (placa.length === 0 || placaInvalida)) || empresa.length === 0}

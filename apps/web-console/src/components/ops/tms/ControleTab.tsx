    ...
    <div className="mt-5 space-y-4">
      ...
      {loading && !data ? <ControlSkeleton /> : <>
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
          <KPIStat label="Volumes no recorte" value={formatInt(totals?.volumes)} hint={`${formatInt(totals?.viagens)} viagens`} icon={Boxes} index={0} />
          <KPIStat label="Recebidos" value={formatInt(totals?.recebidos)} hint="volumes físicos existentes" icon={PackageCheck} index={1} />
...
function AuditTrailModal({ volume, onClose }: { volume: TmsControleVolumeApi; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-[80] grid place-items-center bg-black/75 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Trilha do volume" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <div className="surface-card max-h-[88vh] w-full max-w-2xl overflow-auto p-5 shadow-2xl">
      ...
      {!events ? <p className="py-10 text-center text-sm text-muted-foreground">Carregando eventos...</p> : events.length === 0 ? <p className="py-10 text-center text-sm text-muted-foreground">Este volume ainda não possui eventos adicionais. O recebimento físico é representado pelo próprio cadastro do volume.</p> : <ol className="mt-5 space-y-0">{events.map((event, index) => <li key={event.id} className="grid grid-cols-[18px_minmax(0,1fr)] gap-3"><div className="flex flex-col items-center"><span className={`mt-1.5 h-2.5 w-2.5 rounded-full ${event.tipo === "divergencia" ? "bg-[color:var(--danger)]" : "bg-[color:var(--brand)]"}`} />{index < events.length - 1 && <span className="h-full w-px bg-[color:var(--hairline)]" />}</div><div className="pb-5"><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-sm font-semibold">{labelStatus(event.tipo)}</p><time className="text-[11px] text-muted-foreground">{formatDateTime(event.ocorrido_em ?? event.ocorridoEm)}</time></div><p className="mt-1 text-xs text-muted-foreground">por {event.usuario_nome ?? event.usuarioNome ?? "Usuário não identificado"}</p>{(event.observacao ?? event.obs) && <p className="mt-2 rounded-md bg-[color:var(--surface-tint)] p-2 text-xs">{event.observacao ?? event.obs}</p>}<div className="mt-2 flex gap-3 text-[10px] text-muted-foreground">{(event.foto_url ?? event.fotoUrl) && <span>foto vinculada</span>}{event.gps && <span>GPS registrado</span>}</div></div></li>)}</ol>}
    </div>
  </div>;
}
...
function tripColumns(selectedId: string | null) { return [
  ...
  { key: "funil", header: "Receb. / Embarc. / Entreg.", align: "center" as const, render: (row: TmsControleViagemApi) => <div className="inline-flex items-center gap-1 font-mono text-[11px]"><Count tone="info" value={row.recebidos} /><span className="text-muted-foreground">/</span><Count tone="brand" value={row.embarcados} /><span className="text-muted-foreground">/</span><Count tone="success" value={row.entregues} /></div> },
...
function statusTone(status: string): "danger" | "success" | "brand" | "info" | "warning" | "neutral" { if (status === "divergente") return "danger"; if (status === "entregue") return "success"; if (["embarcado", "reconferido", "desembarcado"].includes(status)) return "brand"; if (status === "conferido") return "info"; return "neutral"; }
...
function downloadCsv(response: TmsControleResponseApi) {
  const header = ["viagem", "embarcacao", "origem", "destino", "saida", "status", "cargas", "volumes", "recebidos", "embarcados", "entregues", "divergentes", "valor_declarado", "valor_cobrado", "cargas_sem_valor_declarado", "cargas_sem_valor_cobrado"];
  const rows = response.items.map((item) => [item.codigo, item.embarcacao_nome, item.origem_sigla, item.destino_sigla, item.data_hora_saida, item.status, item.cargas, item.volumes, item.recebidos, item.embarcados, item.entregues, item.divergentes, item.valor_declarado, item.valor_cobrado, item.cargas_sem_valor_declarado, item.cargas_sem_valor_cobrado]);

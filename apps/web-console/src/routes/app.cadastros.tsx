  ...
  async function salvarFornecedor() {
    ...
    try {
      const created = await createFornecedor({
        ...
        contatos: novoFornecedor.contato
          ? [{ tipo: "whatsapp", valor: novoFornecedor.contato }]
          : [],
      });
    ...
    <AppShell crumb="Cadastros">
      ...
      {tab === "config_operacional" && (
        <div className="mt-5 space-y-4">
          <OperationalTmsConfig />
          <OperationalUnitizacaoConfig />
          <OperationalTmsControlConfig />
      ...
      {tab === "usuarios" && (
        <div className="mt-5 space-y-4">
          <FilterBar
            ...
            right={
              <PrimaryButton icon={Plus} onClick={abrirNovoUsuario}>
                Novo usuario
              </PrimaryButton>
            }
          >
          ...
          {showUsuarioForm && (
            <div className="surface-card grid gap-3 p-4 md:grid-cols-6">
              <input
                ...
                placeholder="Nome"
                className="h-10 rounded-lg bg-[color:var(--muted)] px-3 text-sm ring-1 ring-[color:var(--hairline)] md:col-span-2"
              />
              <input
                value={novoUsuario.login}
                onChange={(e) => setNovoUsuario((v) => ({ ...v, login: e.target.value }))}
                placeholder="Login"
                className="h-10 rounded-lg bg-[color:var(--muted)] px-3 text-sm ring-1 ring-[color:var(--hairline)]"
              />
              <input
                value={novoUsuario.email}
                onChange={(e) => setNovoUsuario((v) => ({ ...v, email: e.target.value }))}
                placeholder="E-mail"
                className="h-10 rounded-lg bg-[color:var(--muted)] px-3 text-sm ring-1 ring-[color:var(--hairline)]"
              />
              <select
                value={novoUsuario.perfilId}
                onChange={(e) => setNovoUsuario((v) => ({ ...v, perfilId: e.target.value }))}
                className="h-10 rounded-lg bg-[color:var(--muted)] px-3 text-sm ring-1 ring-[color:var(--hairline)]"
              >
              ...
              <input
                ...
                onChange={(e) => setNovoUsuario((v) => ({ ...v, password: e.target.value }))}
                placeholder={usuarioEditandoId ? "Nova senha opcional" : "Senha inicial"}
                className="h-10 rounded-lg bg-[color:var(--muted)] px-3 text-sm ring-1 ring-[color:var(--hairline)]"
              />
              <label className="inline-flex h-10 items-center gap-2 text-xs text-muted-foreground">
                <input
                  type="checkbox"
                  checked={novoUsuario.ativo}
                  onChange={(e) => setNovoUsuario((v) => ({ ...v, ativo: e.target.checked }))}
                />
                Ativo
              </label>
              <div className="flex gap-2 md:col-span-5">
                <PrimaryButton icon={Check} disabled={salvando} onClick={salvarUsuario}>
                  {salvando
                    ? "Salvando..."
                    : usuarioEditandoId
                      ? "Salvar usuario"
                ...
                <GhostButton icon={X} onClick={() => setShowUsuarioForm(false)}>
                  Cancelar
                </GhostButton>
              </div>
            </div>
          ...
          <DataTable
            ...
            columns={[
              ...
              },
              {
                key: "login",
                header: "Login",
                render: (r) => <span className="font-mono text-xs">{r.login}</span>,
              },
              ...
              { key: "cidade", header: "Cidade", render: (r) => r.cidade ?? "—" },
              {
                ...
                header: "Status",
                render: (r) => (
                  <StatusChip tone={r.ativo ? "success" : "offline"}>
      ...
      {tab === "perfis" && (
        <div className="mt-5 space-y-4">
          <div className="flex justify-end">
            <PrimaryButton icon={Plus} onClick={abrirNovoPerfil}>
              Novo perfil
            </PrimaryButton>
          </div>
          {showPerfilForm && (
            <div className="surface-card space-y-4 p-4">
              <div className="grid gap-3 md:grid-cols-[1fr_2fr_auto]">
                <input
                  ...
                  onChange={(e) => setNovoPerfil((v) => ({ ...v, nome: e.target.value }))}
                  placeholder="Nome do perfil"
                  className="h-10 rounded-lg bg-[color:var(--muted)] px-3 text-sm ring-1 ring-[color:var(--hairline)]"
                />
                <input
                  value={novoPerfil.descricao}
                  onChange={(e) => setNovoPerfil((v) => ({ ...v, descricao: e.target.value }))}
                ...
                <label className="inline-flex h-10 items-center gap-2 text-xs text-muted-foreground">
                  <input
                    type="checkbox"
                    checked={novoPerfil.ativo}
                    onChange={(e) => setNovoPerfil((v) => ({ ...v, ativo: e.target.checked }))}
                  />
                  Ativo
                </label>
              </div>
              <div className="grid max-h-56 gap-2 overflow-auto sm:grid-cols-2 xl:grid-cols-4">
                {permissoesReais.map((perm) => {
                  ...
                  return (
                    <label
                      key={perm}
                      className="flex items-center gap-2 rounded-md bg-[color:var(--muted)] px-3 py-2 font-mono text-[11px] ring-1 ring-[color:var(--hairline)]"
                    >
                      <input
                        type="checkbox"
                        checked={checked}
                      ...
                      <span>
                        ...
                        <span className="mt-0.5 block font-sans text-[10px] text-muted-foreground">
                          {permissoes.find((item) => item.codigo === perm)?.descricao}
                        </span>
                      </span>
                    </label>
                  );
              ...
              <div className="flex gap-2">
                <PrimaryButton icon={Check} disabled={salvando} onClick={salvarPerfil}>
                  {salvando ? "Salvando..." : perfilEditandoId ? "Salvar perfil" : "Criar perfil"}
                </PrimaryButton>
                <GhostButton icon={X} onClick={() => setShowPerfilForm(false)}>
                  Cancelar
                </GhostButton>
              </div>
            </div>
          ...
          <div className="surface-card overflow-hidden">
            <header className="border-b border-[color:var(--hairline)] px-5 py-4">
              ...
              <p className="mt-0.5 text-xs text-muted-foreground">
                Cada perfil possui permissoes por modulo.acao. Clique no nome do perfil para editar.
              </p>
            </header>
            <div className="overflow-auto">
              <table className="w-full text-sm">
                <thead className="bg-[color:var(--muted)]">
                  <tr>
                    <th className="sticky left-0 z-10 bg-[color:var(--muted)] px-4 py-3 text-left text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground">
                      Permissao
                    </th>
                    {perfis.map((p) => (
                      <th
                        key={p.id}
                        className="px-3 py-3 text-center text-[10px] font-semibold uppercase tracking-[0.12em] text-muted-foreground"
                      >
                        <button onClick={() => editarPerfil(p)} className="hover:text-foreground">
                          {p.nome}
                        </button>
                      </th>
                    ))}
                ...
                <tbody>
                  {permissoesReais.map((perm) => (
                    <tr key={perm} className="border-t border-[color:var(--hairline)]">
                      <td className="sticky left-0 z-10 bg-[color:var(--card)] px-4 py-3">
                      ...
                      {perfis.map((perfil) => {
                        ...
                        return (
                          <td key={perfil.id} className="px-3 py-3 text-center">
                            {has ? (
                              <Check
                                className="mx-auto h-4 w-4 text-[color:var(--success)]"
                                strokeWidth={2.4}
                              />
                            ) : (
                              <X
        ...
        <div className="mt-5 space-y-4">
          <div className="surface-card flex flex-wrap items-center gap-3 p-4">
            ...
            <select className="h-9 rounded-md bg-[color:var(--muted)] px-3 text-sm text-foreground ring-1 ring-[color:var(--hairline)]">
              {cidadesOpcoes.map((c) => (
                <option key={c.sigla}>Saindo de {c.nome}</option>
              ))}
            </select>
            <select className="h-9 rounded-md bg-[color:var(--muted)] px-3 text-sm text-foreground ring-1 ring-[color:var(--hairline)]">
              {cidadesOpcoes.map((c) => (
                <option key={c.sigla}>Indo para {c.nome}</option>
              ))}
            </select>
            <span className="text-xs text-muted-foreground">
              Tabela final será repassada pelo Lucas.
            </span>
          </div>
          <div className="surface-card flex flex-wrap items-center gap-3 p-4">
            ...
            <span className="text-xs text-muted-foreground">
              aplica % a todos os trechos desta tabela.
            </span>
            <div className="ml-auto flex items-center gap-2">
              <input
                ...
                value={reajuste}
                onChange={(e) => {
                  setReajuste(Number(e.target.value));
                  setPreview(false);
                }}
              ...
              <GhostButton icon={ArrowDownUp} onClick={() => setPreview((p) => !p)}>
                {preview ? "Ocultar prévia" : "Pré-visualizar"}
              </GhostButton>
              <PrimaryButton
                icon={TrendingUp}
                disabled={reajustando || reajuste === 0}
                onClick={aplicarReajustePassagem}
              >
          ...
          {preview && reajuste !== 0 && (
            <div className="rounded-md bg-[color:color-mix(in_oklab,var(--brand)_10%,transparent)] px-4 py-2.5 text-xs text-[color:var(--brand)] ring-1 ring-[color:var(--hairline-brand)]">
              Pré-visualização ativa · valores exibidos com {reajuste > 0 ? "+" : ""}
              {reajuste}%. Confirme em "Aplicar reajuste" para gravar a nova versão.
            </div>
          )}
          <DataTable
            rows={precosPassagem.map((p, i) => ({
              ...
              camaroteRoyal:
                p.classes.suite_master ??
                p.classes["suite_master:2_pessoas"] ??
                p.classes.camarote ??
                0,
            }))}
            columns={[
              {
                key: "trecho",
                header: "Trecho",
                render: (r) => <span className="font-display">{r.trecho}</span>,
              },
              {
                key: "rede",
                header: "Rede",
                align: "right",
                render: (r) => <span className="font-mono">{brl(r.rede * fator)}</span>,
              },
              {
                key: "vip",
                header: "Rede VIP",
                align: "right",
                render: (r) => <span className="font-mono">{brl(r.vip * fator)}</span>,
              },
              {
                key: "camaroteRoyal",
                header: "Camarote Royal",
                align: "right",
                render: (r) => <span className="font-mono">{brl(r.camaroteRoyal * fator)}</span>,
              },
            ]}
          />
        </div>
        ...
        <div className="mt-5 space-y-4">
          <div className="surface-card p-4">
            <p className="text-sm font-medium text-foreground">
              Variação por cliente e cidade destino
            </p>
            <p className="mt-1 text-xs text-muted-foreground">
              Tabela de carga pode ser amarrada a cliente específico e variar por
              destino/intertrecho.
            </p>
          </div>
          <DataTable
            rows={precosCarga.map((p, i) => ({
              ...
              tier: p.classe ?? `T${i + 1}`,
              descricao:
                [p.origemSigla, p.destinoSigla].filter(Boolean).join(" -> ") ||
                p.subtipo ||
                "Tabela de carga",
            ...
            columns={[
              {
                key: "tier",
                header: "Tier",
                render: (r) => <span className="font-mono font-semibold">{r.tier}</span>,
              },
              { key: "descricao", header: "Descrição" },
              {
                ...
                render: (r) => (
                  <span className="text-xs text-muted-foreground">
                    {r.idx % 2 === 0 ? "Tabela geral" : "Ferragens Amazônia"}
                  </span>
                ),
              },
              {
                ...
                render: (r) => (
                  <span className="font-mono text-xs">
                    {cidadesOpcoes[(r.idx + 2) % cidadesOpcoes.length]?.sigla ?? "-"}
                  </span>
                ),
              },
              {
                key: "percentual",
                header: "% sobre declarado",
                align: "right",
                render: (r) => (
                  <span className="font-mono font-semibold">{r.percentual.toFixed(1)}%</span>
                ),
              },
            ]}
          />
      ...
      {tab === "fornecedores" && (
        <div className="mt-5 space-y-4">
          <FilterBar
            searchPlaceholder="Buscar fornecedor, CNPJ…"
            right={
              <PrimaryButton icon={Plus} onClick={() => setShowFornecedorForm((value) => !value)}>
                Novo fornecedor
              </PrimaryButton>
            }
          >
          ...
          {showFornecedorForm && (
            <div className="surface-card grid gap-3 p-4 md:grid-cols-[1.2fr_0.9fr_0.9fr_1fr_auto]">
              <input
                ...
                onChange={(event) =>
                  setNovoFornecedor((current) => ({ ...current, nome: event.target.value }))
                }
              ...
              <input
                ...
                onChange={(event) =>
                  setNovoFornecedor((current) => ({ ...current, cnpj: event.target.value }))
                }
                placeholder="CNPJ"
                className="h-10 rounded-md bg-[color:var(--muted)] px-3 text-sm text-foreground ring-1 ring-[color:var(--hairline)] focus:outline-none focus:ring-[color:var(--ring)]"
              />
              <input
                ...
                onChange={(event) =>
                  setNovoFornecedor((current) => ({ ...current, categoria: event.target.value }))
                }
              ...
              <input
                ...
                onChange={(event) =>
                  setNovoFornecedor((current) => ({ ...current, contato: event.target.value }))
                }
                placeholder="WhatsApp/contato"
                className="h-10 rounded-md bg-[color:var(--muted)] px-3 text-sm text-foreground ring-1 ring-[color:var(--hairline)] focus:outline-none focus:ring-[color:var(--ring)]"
              />
              <PrimaryButton icon={Plus} disabled={salvando} onClick={salvarFornecedor}>
                Salvar
              </PrimaryButton>
            </div>
          )}
          <DataTable
            rows={fornecedores.map((f) => ({
              ...
              contato:
                Array.isArray(f.contatos) &&
                f.contatos[0] &&
                typeof f.contatos[0] === "object" &&
                "valor" in f.contatos[0]
            ...
            columns={[
              ...
              },
              {
                key: "cnpj",
                header: "CNPJ",
                render: (r) => <span className="font-mono text-xs">{r.cnpj}</span>,
              },
              {
                key: "categoria",
                header: "Categoria",
                render: (r) => <span className="text-xs">{r.categoria}</span>,
              },
              {
                key: "contato",
                header: "Contato",
                render: (r) => <span className="text-xs text-muted-foreground">{r.contato}</span>,
              },
              {
                key: "ativo",
                header: "Status",
          ...
          <p className="text-center text-[11px] text-muted-foreground">
            Fornecedores vinculam a contas a pagar e compras (Financeiro).
          </p>
        </div>
      )}
      ...
      {tab === "colaboradores" && (
        <div className="mt-5 space-y-4">
          <FilterBar
            searchPlaceholder="Buscar colaborador, função…"
            right={
              <PrimaryButton icon={Plus} onClick={() => setShowColaboradorForm((value) => !value)}>
                Novo colaborador
              </PrimaryButton>
            }
          >
          ...
          {showColaboradorForm && (
            <div className="surface-card grid gap-3 p-4 md:grid-cols-[1.2fr_0.9fr_0.8fr_1fr_auto]">
              <input
                ...
                onChange={(event) =>
                  setNovoColaborador((current) => ({ ...current, nome: event.target.value }))
                }
              ...
              <input
                ...
                onChange={(event) =>
                  setNovoColaborador((current) => ({ ...current, funcao: event.target.value }))
                }
                placeholder="Função"
                className="h-10 rounded-md bg-[color:var(--muted)] px-3 text-sm text-foreground ring-1 ring-[color:var(--hairline)] focus:outline-none focus:ring-[color:var(--ring)]"
              />
              <select
                ...
                onChange={(event) =>
                  setNovoColaborador((current) => ({ ...current, cidadeSigla: event.target.value }))
                }
                className="h-10 rounded-md bg-[color:var(--muted)] px-3 text-sm text-foreground ring-1 ring-[color:var(--hairline)] focus:outline-none focus:ring-[color:var(--ring)]"
              >
                {cidadesOpcoes.map((cidade) => (
                  <option key={cidade.sigla} value={cidade.sigla}>
                    {cidade.sigla}
                  </option>
                ))}
              </select>
              <input
                ...
                onChange={(event) =>
                  setNovoColaborador((current) => ({ ...current, whatsapp: event.target.value }))
                }
                placeholder="WhatsApp"
                className="h-10 rounded-md bg-[color:var(--muted)] px-3 text-sm text-foreground ring-1 ring-[color:var(--hairline)] focus:outline-none focus:ring-[color:var(--ring)]"
              />
              <PrimaryButton icon={Plus} disabled={salvando} onClick={salvarColaborador}>
                Salvar
              </PrimaryButton>
            </div>
          )}
          <DataTable
            ...
            columns={[
              ...
              },
              {
                ...
                header: "CPF",
                render: (r) => (
                  <span className="font-mono text-xs text-muted-foreground">{`000.000.00${r.idx}-0${r.idx}`}</span>
                ),
              },
              {
                key: "funcao",
                header: "Função",
                render: (r) => <span className="text-xs">{r.funcao}</span>,
              },
              { key: "cidade", header: "Cidade" },
              {
                ...
                header: "WhatsApp",
                render: (r) => (
                  <span className="font-mono text-xs text-muted-foreground">{r.whatsapp}</span>
                ),
              },
            ]}
          />
          <p className="text-center text-[11px] text-muted-foreground">
            WhatsApp é usado para notificação de escala (Navegação).
          </p>
        </div>
      )}
    </AppShell>
